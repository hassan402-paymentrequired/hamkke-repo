                            ...
                            <div class="collapse navbar-collapse" id="navbarNav2">
                                <ul class="navbar-nav">
                                    <li class="nav-item @if (empty(request()->tag)) active @endif">
                                        <button
                                            class="nav-link @if (empty(request()->tag)) active @endif d-flex align-items-left align-items-center"
                                    ...
                                    @foreach ($tags as $tag)
                                        <li class="nav-item @if (request()->tag == $tag->slug) active @endif">
                                            <button
                                                class="nav-link @if (request()->tag == $tag->slug) active @endif d-flex align-items-left align-items-center"
                                                id="v-pills-language-tab" type="button">
                                                <a href="{{ route('forum.posts', ['tag' => $tag->slug]) }}">
                                                    {{ $tag->name }} </a></button>
                                        </li>
                                    @endforeach
                                </ul>
                                    ...
                                    <div class="forum card">
                                        <a href="{{ route('forum.posts.view', [$forumPost]) }}">
                                            <div class="space-div">
                                                ...
                                                @endphp
                                                <div
                                                    class="d-flex profile-div align-items-center mt-0 justify-content-sm-between">
